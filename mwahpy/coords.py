...
#=====================================
#COORDINATE/POSITION TRANSFORMATIONS
#=====================================

def XYZ_to_lbr(x, y, z, left_handed=False):
...
def lbr_to_XYZ(l, b, r, left_handed=False):
...
def rotAroundArbAxis(x, y, z, ux, uy, uz, theta):
    ...
    xyz = np.array([x, y, z])
    print('arb. axis =', ux, uy, uz)
    print('before arb. rot. =', xyz)
...
#Below is deprecated version of plane fitter
'''
...
class TestInverses(unittest.TestCase):
    ...
    def test_XYZ_to_lbr_rh(self):
        #right-handed
        x, y, z = 1., 2., 3.
        l, b, r = XYZ_to_lbr(x, y, z)
        new_x, new_y, new_z = lbr_to_XYZ(l, b, r)
        self.assertTrue((round(new_x, prec) == round(x, prec)) and (round(new_y, prec) == round(y, prec)) and (round(new_z, prec) == round(z, prec)))

    def test_XYZ_to_lbr_lh(self):
        #left-handed
        x, y, z = 1., 2., 3.
        l, b, r = XYZ_to_lbr(x, y, z, left_handed=True)
        new_x, new_y, new_z = lbr_to_XYZ(l, b, r, left_handed=True)
        self.assertTrue((round(new_x, prec) == round(x, prec)) and (round(new_y, prec) == round(y, prec)) and (round(new_z, prec) == round(z, prec)))

    def test_XYZ_to_RZphi(self):
        x, y, z = 1., 2., 3.
        R, z, phi = XYZ_to_RZphi(x, y, z)
        new_x, new_y, new_z = RZphi_to_XYZ(R, z, phi)
        self.assertTrue((round(new_x, prec) == round(x, prec)) and (round(new_y, prec) == round(y, prec)) and (round(new_z, prec) == round(z, prec)))
