...
class Timestep():
    ...
    def calc_basic(self):
        ...
        if not 'r' in self._provided_vals or 'r' not in self or self.changed_pos:
        ...
        if not 'l' in self._provided_vals or 'l' not in self or self.changed_pos:
            self.l = np.arctan2(self.y, self.x + 8)*180/np.pi
        if not 'b' in self._provided_vals or 'b' not in self or self.changed_pos:
